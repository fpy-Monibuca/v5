...
type (
	...
	PluginMeta struct {
		...
		KitexRegisterGRPCHandler func(svr server.Server, plugin IPlugin, opts ...server.RegisterOption) error
...
func (p *Plugin) Start() (err error) {
	...
	if p.Meta.KitexServiceDesc != nil && s.kitexServer != nil {
		p.Info("服务注册到kitex")
		// 注册其它service到kitex
		...
		if err = p.Meta.KitexRegisterGRPCHandler(s.kitexServer, p.handler); err != nil {
			p.disable(fmt.Sprintf("grpc %v", err))
			return
		} else {
			p.Info("grpc handler registered")
